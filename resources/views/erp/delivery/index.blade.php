...
<div x-data="deliveryPage()">
    ...
    <div class="mb-4 flex flex-wrap justify-end gap-2">
        ...
        <a href="{{ route('erp.delivery.print') }}" target="_blank"
           class="inline-flex items-center gap-1.5 rounded-lg border border-slate-300 px-3.5 py-2 text-sm font-semibold text-slate-700 transition hover:bg-slate-50">
    ...
    {{-- Add entry --}}
    <form method="POST" action="{{ route('erp.delivery.store') }}" class="mb-6 rounded-2xl border border-slate-200 bg-white p-5">
        @csrf
                            ...
                            <td class="px-4 py-3">
                                <div class="flex items-center justify-end gap-1.5">
                                    <button type="button" title="Receive payment" class="grid h-8 w-8 place-items-center rounded-lg text-slate-500 hover:bg-emerald-50 hover:text-emerald-600"
                                            x-on:click="openPay(@js(['id' => $d->id, 'name' => $d->full_name, 'due' => number_format($due, 2, '.', '')]))"><i class="bi bi-cash-coin"></i></button>
                                    <button type="button" title="Payment history" class="grid h-8 w-8 place-items-center rounded-lg text-slate-500 hover:bg-slate-100 hover:text-slate-700"
                                            x-on:click="openReceipts(@js($d->full_name), @js($receiptRows))"><i class="bi bi-clock-history"></i></button>
    ...
    <div x-show="editing" x-cloak class="fixed inset-0 z-50 flex items-center justify-center p-4">
        ...
        <form method="POST" x-bind:action="updateBase + '/' + form.id" class="relative w-full max-w-2xl rounded-2xl bg-white p-6 shadow-xl">
            ...
            <div class="grid gap-3 sm:grid-cols-2">
                ...
                <div>
                    <label class="{{ $lbl }}">Status <span class="text-rose-500">*</span></label>
                    <select name="status" x-model="form.status" class="{{ $inp }}">
